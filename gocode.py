import sublime, sublime_plugin, subprocess, difflib, threading, os, sys
...
class GocodeExitCommand(sublime_plugin.TextCommand):
	def run(self, edit):
		gocode = subprocess.Popen(["gocode-gomod", "exit"], stdin=subprocess.PIPE, stdout=subprocess.PIPE,env=get_env())
		sout,serr = gocode.communicate()
		pass
...
class GocodeGoreturnsCommand(sublime_plugin.TextCommand):
	def run(self, edit):
		view = self.view
...
class Gocode(sublime_plugin.EventListener):
	...
	def fetch_query_completions(self, view, prefix, location):
		...
		gocode = subprocess.Popen(["gocode-gomod", "-f=csv", "autocomplete", filename, cloc],
			stdin=subprocess.PIPE, stdout=subprocess.PIPE, env=get_env())
	...
	def on_pre_save(self, view):
		if not view.match_selector(0, "source.go"):
			return
		view.run_command('gocode_goreturns')
