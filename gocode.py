import sublime, sublime_plugin, subprocess, difflib, threading, os, time, socket
...
class GocodeGovetCommand(sublime_plugin.TextCommand):
	def run(self, edit):
		view = self.view
		# print(os.path.dirname(view.file_name()))
	...
	def do_vet(self, work_dir):
		govet = subprocess.Popen(["go","vet","."], stdout=subprocess.PIPE, stderr=subprocess.PIPE, creationflags=0x08000000, cwd=work_dir)
...
class Gocode(sublime_plugin.EventListener):
	...
	def fetch_query_completions(self, view, prefix, location):
		...
		gocode = subprocess.Popen([self.GOCODE,"-builtin", "-ignore-case","-unimported-packages", "-f=csv", "autocomplete", filename, cloc], stdin=subprocess.PIPE, stdout=subprocess.PIPE, creationflags=0x08000000)
	...
	def on_activated_async(self, view):
		...
		if self.watchers_running == False:
			thread1 = threading.Thread(target=self.watch_gocode,)
			thread1.start()
			thread2 = threading.Thread(target=self.run_gocode_watcher,)
			thread2.start()
			self.watchers_running=True

	
	def run_gocode_watcher(self):
		subprocess.call("keepalive", creationflags=0x08000000,shell=False)

	def watch_gocode(self):
		HOST = 'localhost'  # Standard loopback interface address (localhost)
		PORT = 22345        # Port to listen on (non-privileged ports are > 1023)
